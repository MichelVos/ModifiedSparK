...
def load_checkpoint(resume_from: str, model_without_ddp, optimizer):
    ...
    # Get last file indicated with highest epoch numer at the end of the filenael, e.g., `model_withdecoder_1kpretrained_spark_style_123.pth` => 123
    # First get all files with prefix `model_withdecoder_1kpretrained_spark_style_` and suffix `.pth`
    # resume_from contains a filename without the epoch at the end, e.g., `model_withdecoder_1kpretrained_spark_style.pth` => find the file with the highest epoch number at the end
    # first create a mask
    mask = os.path.basename(resume_from).replace(os.path.splitext(resume_from)[1], '') + '_*.pth'
    mask = os.path.join(os.path.dirname(resume_from), mask)
    ...
    if files:
        ...
        if max_file is not None:
            resume_from = max_file
    if len(resume_from) == 0:
        return 0, '[no performance_desc]'
    if not os.path.isfile(resume_from):
        print(f'[load_checkpoint] no checkpoint found at `{resume_from}`. Start from scratch.')
        return 0, '[no performance_desc]'
    print(f'[try to resume from file `{resume_from}`]')
    checkpoint = torch.load(resume_from, map_location='cpu')
    

    has_w = any(k.endswith("densify_projs.3.weight") for k in checkpoint["module"].keys())
    has_b = any(k.endswith("densify_projs.3.bias") for k in checkpoint["module"].keys())

    print("ckpt has densify_projs.3.weight:", has_w)
    print("ckpt has densify_projs.3.bias:", has_b)
    ...
    print("Checkpoint param groups:", len(checkpoint['optimizer']['param_groups']))
    print("Current optimizer param groups:", len(optimizer.param_groups))
    all_groups_match = True
    for i, (g_ckpt, g_opt) in enumerate(zip(checkpoint['optimizer']['param_groups'], optimizer.param_groups)):
        if set(g_ckpt.keys()) != set(g_opt.keys()):
            all_groups_match = False
            print(f"Param group {i} keys do not match!")
            print(f"  Checkpoint keys: {set(g_ckpt.keys())}")
            print(f"  Optimizer keys:  {set(g_opt.keys())}")
    for i, g in enumerate(optimizer.param_groups):
        print(f"Current group {i}: {len(g['params'])} params")

    for i, g in enumerate(checkpoint['optimizer']['param_groups']):
        print(f"Checkpoint group {i}: {len(g['params'])} params")

    if all_groups_match and 'optimizer' in checkpoint:
