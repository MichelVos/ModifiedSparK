...
def main_pt():
    ...
    data_loader_train = DataLoader(
        ...
        batch_sampler=DistInfiniteBatchSampler(
            ...
            shuffle=True, filling=True, rank=dist.get_rank(), world_size=dist.get_world_size(), seed = args.seed
    ...
    model_without_ddp = SparK(
        sparse_encoder=enc, dense_decoder=dec, mask_ratio=args.mask, mask_type = args.mask_type, mask_area=args.mask_area,
        densify_norm=args.densify_norm, sbn=args.sbn, exportDirectory=args.exp_dir, mean=dataset_train.mean, std=dataset_train.std, device=args.device
    ).to(args.device)
    print("model param device:", next(model_without_ddp.parameters()).device)
    ...
    if ep_start >= args.ep: # load from a complete checkpoint file
        ...
        for ep in range(ep_start, args.ep):
            print(f"Epoch {ep} start time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
            ep_start_time = time.time()
            tb_lg.set_step(ep * iters_train)
            if hasattr(itrt_train, 'set_epoch'):
                itrt_train.set_epoch(ep)
            model_without_ddp._vis_dumped = False
            model_without_ddp.picture_number = ep
            ...
            misc.save_checkpoint_with_meta_info_and_opt_state(f'{args.model}_withdecoder_1kpretrained_spark_style_{ep}.pth', args, ep, performance_desc, model_without_ddp.state_dict(with_config=True), optimizer.state_dict())
            misc.save_checkpoint_model_weights_only(f'{args.model}_1kpretrained_timm_style_{ep}.pth', args, model_without_ddp.sparse_encoder.sp_cnn.state_dict())
...
def pre_train_one_ep(ep, args: arg_util.Args, tb_lg: misc.TensorboardLogger, itrt_train, iters_train, model: DistributedDataParallel, optimizer):
    ...
    for it, inp in enumerate(me.log_every(iters_train, itrt_train, 3, header)):
        ...
        #print(f"{it}/{iters_train}", flush=True)
        # adjust lr and wd
        ...
        SparK.forward
        visualize = True
        loss = model(inp, active_b1ff=None, vis=visualize)
        optimizer.zero_grad()
        loss.backward()
        loss = loss.item()
        ...
        for p in model.parameters():
            if p.grad is not None:
                #print(f'[rk{dist.get_rank():02d}] grad of param {p.shape} has mean {p.grad.mean():.4e} and std {p.grad.std():.4e}', force=True, flush=True)
                if not torch.isfinite(p.grad).all():
                    print("NaN/Inf in gradients")
                    sys.exit(-1)        
        # optimize
        grad_norm = None
        if early_clipping: grad_norm = torch.nn.utils.clip_grad_norm_(params_req_grad, args.clip).item()
        '''
        g = model.sp_cnn.conv1.weight.grad[0].detach().cpu().numpy()
        plt.imshow(g[0], cmap='gray')
        plt.title("Gradient of first kernel")
        plt.colorbar()
        plt.show()        
        '''
        optimizer.step()
