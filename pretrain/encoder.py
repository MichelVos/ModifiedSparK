...
def _get_active_ex_or_ii(H, W, returning_active_ex=True):
    ...
    #print(f"H={H}, W={W}, _cur_active.shape={_cur_active.shape}, h_repeat={h_repeat}, w_repeat={w_repeat}")
...
def sp_conv_forward(self, x: torch.Tensor):
    ...
    active_ex_or_ii = _get_active_ex_or_ii(H=x.shape[2], W=x.shape[3], returning_active_ex=True)    # (BCHW) *= (B1HW), mask the output of conv
    #print(active_ex_or_ii.shape)
    x *= active_ex_or_ii
    return x
